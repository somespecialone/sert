<script lang='ts'>
	import ExpiredIco from '@/components/icons/ExpiredIco.svelte';
...
<div class='Converter'>
	{#each Object.entries(values) as [currency, value], i (currency)}
		<div class='RateLine' data-focused='{focusedCurrency === currency ? "" : null}'>
			{#if (currency !== 'USD')}
			...
			/>
			{#if (currency !== 'USD' && isExpired(rates[currency][1]))}
				<ExpiredIco />
			{/if}
		</div>
	{/each}
