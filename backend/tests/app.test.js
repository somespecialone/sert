...
const { RATES_PREFIX, HISTORY_PREFIX } = require("../src/constants");
...
beforeAll(async () => {
  ...
  await db.put({ rate: MOCK_CURRENCY_HISTORY[0][0], updated }, RATES_PREFIX + MOCK_CURRENCY);
  await db.put({ history: MOCK_CURRENCY_HISTORY, updated }, HISTORY_PREFIX + MOCK_CURRENCY);
});

afterAll(async () => {
  await db.delete(RATES_PREFIX + MOCK_CURRENCY);
  await db.delete(HISTORY_PREFIX + MOCK_CURRENCY);
});
...
describe("Test main routes", () => {
  ...
  test("History route", async () => {
    ...
    expect(res.status).toEqual(200);
    expect(res.headers["content-type"]).toMatch(/json/);
    expect(res.body[MOCK_CURRENCY]).toEqual(
      MOCK_CURRENCY_HISTORY.slice(0, Number(process.env.HISTORY_LENGTH || 30))
    );
  });
  test("History route with length", async () => {
    const res = await request(app).get("/api/history?length=15");
    expect(res.status).toEqual(200);
    expect(res.headers["content-type"]).toMatch(/json/);
